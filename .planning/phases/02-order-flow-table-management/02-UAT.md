...
number: 12
name: Void an Entire Order (Manager PIN)
expected: |
  A "Void Order" button appears in the order panel footer for open orders. Tapping
  it opens the VoidReasonDialog. After selecting a reason and entering manager PIN
  1234, the entire order is voided — all items removed, table returns to free (green)
  on /tables.
...
result: pass
note: Fixed Turbopack panic caused by &middot; HTML entity — replaced with unicode ·
...
result: pass
note: Fixed — added min-h-0 to ScrollArea so footer stays pinned; also added overflow-hidden to aside containers
